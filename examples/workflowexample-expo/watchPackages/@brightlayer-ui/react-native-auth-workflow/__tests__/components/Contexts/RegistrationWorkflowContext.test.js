...
describe('RegistrationWorkflowContext', () => {
    it('should render RegistrationWorkflowContextProvider without crashing', () => {
        render(_jsx(RegistrationWorkflowContextProvider, { ...registrationWorkflowContextProps, children: _jsx(Text, { children: "Test" }) })).toJSON();
        expect(render).toBeTruthy();
        expect(screen.getByText('Test')).toBeTruthy();
    });
    it('should set values in the context', async () => {
        let values;
        const RegisterComponent = () => (_jsx(RegistrationWorkflowContextProvider, { ...registrationWorkflowContextProps }));
        const CustomFlow = () => {
            const Screen1 = () => {
        ...
        // eslint-disable-next-line
        await (() => expect(values.result.current.screenData.CreateAccount.emailAddress).toBe('emailAddress@emailAddress.emailAddress'));
    });
