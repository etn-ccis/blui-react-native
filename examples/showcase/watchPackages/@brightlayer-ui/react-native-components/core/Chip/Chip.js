...
export const Chip = (props) => {
    const { children, icon, iconColor, style, textStyle, mode = 'outlined', selected, disabled, avatar, chipColor, borderColor, textColor, theme: themeOverride, styles = {}, ...rest } = props;
    ...
    const getIcon = () => {
        if (icon) {
            return (React.createElement(Icon, { source: icon, size: 18, color: iconColor ? iconColor : textColor ? textColor : DefaultTextColor }));
        }
        else if (avatar) {
    ...
    const renderCloseIcon = () => (React.createElement(Icon, { source: { name: 'close' }, size: 18, color: DefaultTextColor }));
    const renderIcon = () => getIcon();
    return (React.createElement(React.Fragment, null, icon ? (React.createElement(PaperChip, { style: [
            {
            ...
            style,
            chipStyle,
        ], textStyle: [{ color: textColor ? textColor : DefaultTextColor, ...fontStyleRegular }, textStyle], showSelectedCheck: false, selected: selected, disabled: disabled, ...(isElevated && { elevated: !disabled }), closeIcon: renderCloseIcon, icon: renderIcon, ...rest }, children)) : avatar ? (React.createElement(PaperChip, { style: [
            {
                backgroundColor: chipColor ? chipColor : defaultChipColor,
            ...
            style,
            chipStyle,
        ], textStyle: [{ color: textColor ? textColor : DefaultTextColor, ...fontStyleRegular }, textStyle], showSelectedCheck: false, selected: selected, disabled: disabled, avatar: React.cloneElement(avatar, {
            style: avatar.props.size
                ...
                : {},
        }), ...(isElevated && { elevated: !disabled }), closeIcon: renderCloseIcon, ...rest }, children)) : (React.createElement(PaperChip, { style: [
            {
                backgroundColor: chipColor ? chipColor : defaultChipColor,
            },
            styles.root,
            style,
            chipStyle,
        ], textStyle: [{ color: textColor ? textColor : DefaultTextColor, ...fontStyleRegular }, textStyle], showSelectedCheck: false, selected: selected, disabled: disabled, ...(isElevated && { elevated: !disabled }), closeIcon: renderCloseIcon, ...rest }, children))));
};
