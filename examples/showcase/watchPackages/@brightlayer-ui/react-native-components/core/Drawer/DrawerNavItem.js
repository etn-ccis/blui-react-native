...
const calcNestedPadding = (depth, insets) => insets.left + 16 + (depth > 0 ? depth * 40 : 0);
const makeStyles = (props, theme, fontScale) => {
    const { 
    // Shared style props
    activeItemBackgroundColor = theme.colors.primaryContainer, activeItemBackgroundShape = 'square', backgroundColor, depth, nestedBackgroundColor = theme.colors.surfaceContainer, } = props;
...
export const DrawerNavItem = (props) => {
    ...
    const { 
    // Shared style props
    activeChevronColor = theme.colors.onPrimaryContainer, activeItemBackgroundColor /* eslint-disable-line @typescript-eslint/no-unused-vars */, activeItemBackgroundShape /* eslint-disable-line @typescript-eslint/no-unused-vars */, activeItemFontColor = theme.colors.onPrimaryContainer, activeItemIconColor = theme.colors.onPrimaryContainer, backgroundColor /* eslint-disable-line @typescript-eslint/no-unused-vars */, chevron, chevronColor = theme.colors.onSurfaceVariant, collapseIcon = { family: 'material', name: props.depth ? 'arrow-drop-up' : 'expand-less' }, disableActiveItemParentStyles = false, divider, expandIcon = { family: 'material', name: props.depth ? 'arrow-drop-down' : 'expand-more' }, hidePadding, itemFontColor = theme.colors.onSurface, itemIconColor = theme.colors.onSurfaceVariant, nestedBackgroundColor /* eslint-disable-line @typescript-eslint/no-unused-vars */, nestedDivider, 
    // Drawer Nav Item specific props
    children, styles = {}, depth = 0, hidden, icon: itemIcon, InfoListItemProps = {}, isInActiveTree, itemID, items, notifyActiveParent = () => { }, onPress, rightComponent, statusColor, subtitle: itemSubtitle, title: itemTitle,
    // other View props
     } = otherProps;
    const defaultProps = {
    ...
    const showDivider = depth > 0 ? (nestedDivider !== undefined ? nestedDivider : false) : divider !== undefined ? divider : false;
    ...
    const getActionComponent = useCallback(() => {
        ...
        return (React.createElement(View, { style: [defaultStyles.expandIcon, styles.expandIcon] },
            React.createElement(Icon, { source: collapseIcon && expanded ? collapseIcon : expandIcon, size: 24, color: theme.colors.onSurfaceVariant, allowFontScaling: !disableScaling })));
    }, [
        items,
        children,
        styles,
    ...
    const getChildren = useCallback(() => findChildByType(children, ['DrawerNavItem'])
        // .slice(0, 1)
    ...
    return (React.createElement(React.Fragment, null, !hidden && (React.createElement(React.Fragment, null,
        React.createElement(View, { style: [defaultStyles.root, styles.root] },
            active && React.createElement(View, { style: [defaultStyles.activeBackground, styles.activeBackground] }),
            React.createElement(InfoListItem, { dense: true, title: itemTitle, subtitle: itemSubtitle, divider: showDivider ? 'full' : undefined, statusColor: statusColor, fontColor: active ? activeItemFontColor : itemFontColor, icon: icon, chevron: !items && !children ? chevron : false, chevronColor: active ? activeChevronColor : chevronColor, iconColor: active ? activeItemIconColor : itemIconColor, rightComponent: (actionComponent || rightComponent) && (React.createElement(View, { style: { flexDirection: 'row', alignItems: 'center' } },
                    rightComponent,
                    actionComponent)), backgroundColor: 'transparent', onPress: hasAction ? onPressAction : undefined, hidePadding: hidePadding, styles: {
                    root: Object.assign({ paddingLeft: calcNestedPadding(depth, insets) }, iliRoot),
                    title: Object.assign(active || (isInActiveTree && !disableActiveItemParentStyles)
                        ? {
                            ...fontStyleSemiBold,
        ...
        ((items && items.length > 0) || Boolean(children)) && (React.createElement(Collapsible, { collapsed: !expanded },
            items?.map((subItem, index) => (React.createElement(DrawerNavItem, { key: `itemList_${index}`, ...subItem, ...inheritSharedProps({ ...defaultProps, ...props }, subItem), depth: depth + 1, isInActiveTree: activeHierarchy.includes(subItem.itemID), notifyActiveParent: (ids = []) => {
                    notifyActiveParent(ids.concat(itemID));
                } }))),
            getChildren()))))));
};
