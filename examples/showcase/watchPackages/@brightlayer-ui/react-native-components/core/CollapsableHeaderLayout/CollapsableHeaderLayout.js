...
const useUpdateScrollView = (contentPaddingValue, contentPadding, animatePadding, scrollRef, scrollValue) => useMemo(() => {
    const updateScrollView = (data) => {
        const { padding, animate, scrollTo } = data;
        if (padding !== null && padding !== contentPaddingValue) {
            if (animate)
                animatePadding(padding).start();
            else
                contentPadding.setValue(padding);
        }
...
export const CollapsibleHeaderLayout = (props) => {
    const { HeaderProps = { styles: {} }, theme: themeOverride, ScrollViewProps = {}, styles = {}, style, ScrollComponent, ...viewProps } = props;
    ...
    // State Variables
    const [contentPadding] = useState(new Animated.Value(headerVariant === 'dynamic' || startExpanded || false ? expandedHeight : collapsedHeight));
    const [contentPaddingValue, setContentPaddingValue] = useState(headerVariant === 'dynamic' || startExpanded || false ? expandedHeight : collapsedHeight);
    // Animation function to smoothly animate the paddingTop of ScrollView
    const animatePadding = (padding) => Animated.timing(contentPadding, {
        toValue: padding,
    ...
    const updateScrollView = useUpdateScrollView(contentPaddingValue, contentPadding, animatePadding, scrollRef, scrollValue);
    ...
    return (React.createElement(View, { ...viewProps, style: [{ flex: 1, backgroundColor: theme.colors.background }, styles.root, style] },
        React.createElement(Header, { testID: 'blui-header', ...HeaderProps, updateScrollView: updateScrollView, scrollPosition: animatedScrollValue, style: [
