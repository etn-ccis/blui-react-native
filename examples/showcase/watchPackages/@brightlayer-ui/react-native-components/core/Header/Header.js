...
import { Animated, SafeAreaView, StatusBar, TouchableWithoutFeedback, Platform, } from 'react-native';
...
export const Header = (props) => {
    const { actionItems, actionItemColor, backgroundColor, backgroundImage, expandable = false, expandedHeight: expandedHeightProp = 200, collapsedHeight: collapsedHeightProp = 56, fontColor, info, icon, navigationIconColor, onIconPress, scrollPosition = new Animated.Value(0), searchableConfig, startExpanded, subtitle, style, styles = {}, theme: themeOverride, title, variant = 'static', updateScrollView = () => { }, ...viewProps } = props;
    ...
    const [staticHeaderHeight] = useState(startExpanded ? new Animated.Value(expandedHeight) : new Animated.Value(collapsedHeight));
    ...
    const getActionItemInfo = useCallback(() => {
        ...
        const componentsWidth = actionComponents.reduce((accumulator, currentValue) => accumulator + (currentValue.width || 40 * fontScale), 0);
        return {
    ...
    useEffect(() => {
        ...
        // Going from static -> dynamic
            ...
            if (!inDynamicRange) {
                ...
                if (expanded) {
                    staticHeaderHeight.setValue(scrollPositionValue <= scrollableDistance / 2 ? expandedHeight : collapsedHeight);
                    updateScrollView({
                        padding: expandedHeight,
                        animate: false,
    ...
    useEffect(() => {
        ...
        if (previousExpandedHeight === undefined || previousCollapsedHeight === undefined)
            return;
    ...
    const getDynamicHeaderHeight = () => dynamicHeaderHeight.interpolate({
    ...
    const statusBarStyle = useCallback(() => color(getBackgroundColor()).isDark() ? 'light-content' : 'dark-content', [getBackgroundColor]);
    ...
    const onChangeSearchText = useCallback((text) => {
        setQuery(text);
        if (searchableConfig?.onChangeText)
            searchableConfig.onChangeText(text);
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(StatusBar, { barStyle: statusBarStyle(), translucent: true, backgroundColor: 'transparent' }),
        React.createElement(TouchableWithoutFeedback, { accessible: false, onPress: () => onPress(), disabled: !expandable || searching, ...viewProps },
            React.createElement(AnimatedSafeAreaView, { style: [
                    defaultStyles.root,
                    styles.root,
                    style,
                ...
                React.createElement(SearchContext.Provider, { value: {
                    ...
                    React.createElement(ColorContext.Provider, { value: { color: getFontColor() } },
                        React.createElement(HeaderHeightContext.Provider, { value: {
                                headerHeight: useStaticHeight ? staticHeaderHeight : getDynamicHeaderHeight(),
                            } },
                            React.createElement(HeaderBackgroundImage, { backgroundImage: backgroundImage, style: styles.backgroundImage }),
                            React.createElement(Animated.View, { style: [contentStyle(), styles.content] },
                                React.createElement(HeaderNavigationIcon, { icon: icon, navigationIconColor: navigationIconColor ? navigationIconColor : theme.colors.onSurface, onPress: onIconPress, style: styles.icon }),
                                React.createElement(HeaderContent, { theme: theme, title: title, subtitle: subtitle, info: info, actions: getActionItemInfo(), styles: {
                                ...
                                React.createElement(HeaderActionItems, { actionItems: actionItems, actionItemColor: actionItemColor ? actionItemColor : theme.colors.onSurfaceVariant, styles: {
