...
const filledBodyPath =
    'M4 6H20C21.1046 6 22 6.89543 22 8V18C22 19.1046 21.1046 20 20 20H4C2.89543 20 2 19.1046 2 18V8C2 6.89543 2.89543 6 4 6Z';
const clipId = 'blui-battery-large-clip';
const filledBodyClipId = 'blui-battery-large-clip-filled-body';
...
export const BatteryLarge: React.FC<BatteryLargeProgressProps> = (props) => {
    ...
    // Tuck the outlined fill slightly under the outline border to avoid an Android anti-aliasing seam
    const overlap = outlined ? 0.5 : 0;
    const topOverlap = outlined && rangedPercent >= 100 ? overlap : 0;
    // For the filled variant, only include the terminals in the fill at 100%; below that fill the body only
    const filledHeight = rangedPercent >= 100 ? 16 : (rangedPercent / 100) * 14;
    const filledY = rangedPercent >= 100 ? 4 : 20 - filledHeight;
    const fillHeight = outlined ? (rangedPercent / 100) * 10 + overlap + topOverlap : filledHeight;
    const fillY = outlined ? 8 + ((100 - rangedPercent) / 100) * 10 - topOverlap : filledY;
    const fillX = outlined ? 4.01 - overlap : 2;
    const fillWidth = outlined ? 16 + overlap * 2 : 20;
    const baseFillOpacity = outlined || backgroundColor || percent >= 100 ? 1 : 0.3;
    const filledProgressClipPath = rangedPercent >= 100 ? `url(#${clipId})` : `url(#${filledBodyClipId})`;
        ...
        >
            <Svg height={`${size}px`} width={`${size}px`} x="0px" y="0px" viewBox="0 0 24 24" {...svgProps}>
                ...
                {outlined ? (
                    <>
                        {rangedPercent > 0 && (
                            <Rect x={fillX} y={fillY} width={fillWidth} height={fillHeight} fill={color} />
                        )}
                        {/* Draw the outline on top so its border hides the fill overlap and the seam */}
                        <Path d={outlinedPath} fill={color} fillOpacity={1} fillRule="evenodd" clipRule="evenodd" />
                    </>
                ) : (
                    <>
                        <Path
                        ...
                        {rangedPercent > 0 && (
                            <Rect
                                ...
                                height={fillHeight}
                                fill={color}
                                clipPath={filledProgressClipPath}
                            />
                        )}
                    </>
                )}
