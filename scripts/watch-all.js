...
const PACKAGES = {
    components: {
        ...
        exampleApps: ['examples/showcase', 'examples/designPatterns']
    },
    themes: {
        name: 'Themes',
        color: colors.green,
    ...
    workflow: {
        ...
        exampleApps: ['examples/workflowexample-expo']
    },
    progressIcons: {
        name: 'Progress Icons',
...
class WatchAllApp {
    ...
    async initialize() {
        ...
        log(`${colors.yellow}📦 Packages: ${Object.keys(PACKAGES).map(key => PACKAGES[key].name).join(', ')}${colors.reset}`);
        ...
        if (successCount === Object.keys(PACKAGES).length) {
            ...
            log(`${colors.blue}💡 Tip: Save any file in any package src/ to trigger auto-build and sync to example apps${colors.reset}`);
        } else {
