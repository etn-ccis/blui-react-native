...
// Configuration
const THEMES_LIBRARY_PATH = path.resolve(__dirname, '../packages/themes');
const SRC_PATH = path.join(THEMES_LIBRARY_PATH, 'src');
const DIST_PATH = path.join(THEMES_LIBRARY_PATH, 'dist');
const TSCONFIG_PATH = path.join(THEMES_LIBRARY_PATH, 'tsconfig.json');

// Example apps that need to be updated
const EXAMPLE_APPS = [
    'examples/showcase',
    'examples/designPatterns',
    'examples/showcase-expo',
];

const NODE_MODULES_PATH = 'watchPackages/@brightlayer-ui/react-native-themes';
...
function buildSingleFile(srcFilePath) {
    return new Promise((resolve, reject) => {
        logInfo(`Building file: ${path.relative(THEMES_LIBRARY_PATH, srcFilePath)}`);

        try {
            // Get the relative path for the TypeScript compiler
            const relativeSrcPath = path.relative(THEMES_LIBRARY_PATH, srcFilePath);
