...
export const InternalLink = (props: LinkProps): JSX.Element => {
  ...
  return (
    <Link
      rel={'noopener noreferrer'}
      style={{fontWeight: 400, color: theme.palette.primary.main}}
      {...props}
    />
  );
};
...
export const componentsMap = {
  ...
  a: (props: React.LinkHTMLAttributes<HTMLLinkElement>): JSX.Element => {
    ...
    if (
      props.href &&
      (props.href.match(/^http/gi) || props.href.match(/^mailto/gi))
    ) {
  ...
  ul: (props: BoxProps): JSX.Element => (
    <Box component={'ul'} sx={{m: '15px auto', pl: '60px'}} {...props} />
  ),
    ...
    <Typography
      ...
      sx={{
        ...
        border: (theme: Theme) =>
          theme.palette.mode === 'light'
            ? undefined
            : `${theme.palette.divider} 1px solid`,
      }}
      {...props}
    />
  ),
