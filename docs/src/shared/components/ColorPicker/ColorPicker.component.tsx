...
const MUI_COLOR =
  /^(primary|secondary|error|success|warning|info|text)\.(main|dark|light|contrastText)$/;
const MUI_GREY =
...
export const ColorPicker = (props: ColorPickerProps): JSX.Element => {
  ...
  const validColor =
    colorIsValid(color, allowMUIColors) || (!propData.required && color === '');

  // debounce the color change events to 1 per 300ms
  const handleColorChangeDebounced = useMemo(
    () => _debounce(handleColorChange, 300),
    [handleColorChange],
  );
  ...
  return (
    <>
      <TextField
        ...
        error={!validColor}
        helperText={
          validColor ? propData.helperText : 'Color value not recognized'
        }
      />
    </>
  );
