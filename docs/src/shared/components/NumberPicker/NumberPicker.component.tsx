...
const floatMath = (value: string | number, step: number): number => {
  ...
  const floatCorrectionFactor = Math.pow(
    10,
    Math.max(valueDecimalPlaces, stepDecimalPlaces),
  );
  // floating point math correction
  return (
    ((value as number) * floatCorrectionFactor + step * floatCorrectionFactor) /
    floatCorrectionFactor
  );
};
      ...
      <Collapse
        ...
        <Paper square sx={{p: 1, width: '100%'}}>
          <ClickAwayListener onClickAway={handleClose}>
            <Slider
              value={
                propData.inputValue === '' ? 0 : (propData.inputValue as number)
              }
              size={'small'}
              ...
              onChange={(event, value): void =>
                handleNumberChange({target: {value: value}} as any)
              }
            />
          </ClickAwayListener>
        </Paper>
