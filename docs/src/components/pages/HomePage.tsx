...
const styles = {
    pageRoot: {
        background: (theme: Theme): string =>
            `url("${CirclesImage.toString()}") no-repeat center/200%, ${theme.palette.primary.main}`,
        minHeight: '100%',
    ...
    return (
        <Box sx={styles.pageRoot}>
            ...
            <Stack sx={styles.content} gap={8}>
                ...
                <Stack direction={'row'} gap={lgUp ? 5 : 4} flexWrap={'wrap'} sx={{ marginTop: 5 }}>
                    <Stack sx={styles.packageStats}>
                        <Typography variant={'body1'}>Total React Native Packages</Typography>
                ...
                <Stack direction={'row'} gap={2} flexWrap={'wrap'}>
                    <Button
                        href={`${import.meta.env.BASE_URL ? import.meta.env.BASE_URL : ''}/getting-started/environment`}
                        variant={'contained'}
                        disableElevation
