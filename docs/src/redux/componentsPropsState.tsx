...
export const componentPropsStateSlice = createSlice({
  ...
  reducers: {
    ...
    updateProp: (state, action: PayloadAction<ComponentDocPayloadType>) => {
      ...
      const updatedKnob = newArray?.props?.filter(
    ...
    updateSharedProp: (
      state,
      action: PayloadAction<ComponentDocPayloadType>,
    ) => {
      const newArray = getComponentState(action.payload.componentName, state);
      const updatedKnob = newArray?.sharedProps?.filter(
        prop => prop.propName === action.payload.propName,
      );
      if (updatedKnob) {
        updatedKnob[0].inputValue = action.payload.propValue;
    ...
    updateOtherProp: (
      ...
      const newArray = getComponentState(action.payload.componentName, state);
      const updatedKnob = newArray?.otherProps?.filter(
        prop => prop.propName === action.payload.propName,
      );
      if (updatedKnob) {
        updatedKnob[0].inputValue = action.payload.propValue;
    ...
    updateOtherComponentProp: (
      ...
      const newArray = getComponentState(action.payload.componentName, state);
      const updatedKnob =
        newArray?.otherComponentProps?.childComponentProps?.filter(
          prop => prop.propName === action.payload.propName,
        );
      if (updatedKnob) {
        updatedKnob[0].inputValue = action.payload.propValue;
